...
export const RouteDisplay: React.FC<RouteDisplayProps> = ({
  ...
  return (
    <motion.div
      className={cn(
        'bg-kawaii-neutral-100 dark:bg-kawaii-neutral-800/50',
        'rounded-xl p-3 border border-kawaii-neutral-200 dark:border-kawaii-neutral-700',
        className
      )}
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
          ...
          {stops.map((stop, index) => (
            <React.Fragment key={index}>
              ...
              {index < stops.length - 1 && (
                <span className="text-kawaii-neutral-400 dark:text-kawaii-neutral-600">
                  →
                </span>
              )}
            </React.Fragment>
        ...
        {/* Travel Time */}
        {travelTimeText && (
          <span className="text-xs px-2 py-1 rounded-full bg-white dark:bg-kawaii-neutral-700 text-kawaii-neutral-600 dark:text-kawaii-neutral-400 whitespace-nowrap">
            {travelTimeText}
          </span>
        )}
      </div>
