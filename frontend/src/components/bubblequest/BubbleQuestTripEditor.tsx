...
import { Button as BubbleQuestButton } from './Button';
import { Input as KawaiiInput } from './Input';

interface BubbleQuestTripEditorProps {
  trip?: Trip;
  mode: 'create' | 'edit';
...
export const BubbleQuestTripEditor: React.FC<BubbleQuestTripEditorProps> = ({
  trip,
  mode,
  onSave,
  onCancel,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        />
        <div>
          <label className="block text-sm font-medium mb-2 bubblequest-text">
            Currency 💱
          </label>
          <select
            name="currency_code"
            ...
            className="w-full px-4 py-3 rounded-2xl border-2 border-bubblequest-purple/20 
              focus:border-bubblequest-purple focus:ring-2 focus:ring-bubblequest-purple/20
              bg-white/80 backdrop-blur-sm transition-all duration-200
              hover:border-bubblequest-purple/40"
          >
      ...
      {/* Theme Selector */}
      <div>
        <label className="block text-sm font-medium mb-3 bubblequest-text">
          Choose Your Vibe ✨
        </label>
              ...
              className={`
                ...
                ${
                  values.theme === theme.value
                    ? 'border-bubblequest-purple bg-gradient-to-br ' + theme.color + ' text-white shadow-lg scale-105'
                    : 'border-bubblequest-purple/20 bg-white/80 backdrop-blur-sm hover:border-bubblequest-purple/40'
                }
              `}
            >
      ...
      <div>
        ...
        <div className="space-y-3">
          {imagePreview && (
            <div className="relative w-full h-48 rounded-2xl overflow-hidden border-2 border-bubblequest-purple/20">
              <img
                src={imagePreview}
                alt="Cover preview"
              ...
              <button
                ...
                className="absolute top-2 right-2 bg-bubblequest-pink text-white p-2 rounded-full 
                  hover:bg-bubblequest-pink/80 transition-all duration-200 hover:scale-110"
              >
          ...
          <label className="block">
            <input
              ...
              className="hidden"
            />
            <div className="px-4 py-3 rounded-2xl border-2 border-dashed border-bubblequest-purple/30 
              bg-white/80 backdrop-blur-sm cursor-pointer
              hover:border-bubblequest-purple hover:bg-bubblequest-purple/5 transition-all duration-200
              text-center">
              <span className="text-bubblequest-purple font-medium">
      ...
      {/* Privacy Settings */}
      <div className="space-y-3 p-4 rounded-2xl bg-gradient-to-br from-bubblequest-purple/5 to-bubblequest-pink/5 border border-bubblequest-purple/10">
        <label className="flex items-center space-x-3 cursor-pointer group">
          <input
            ...
            onChange={handleInputChange}
            className="w-5 h-5 text-bubblequest-purple border-bubblequest-purple/30 rounded-lg 
              focus:ring-bubblequest-purple/20 focus:ring-2"
          />
          <span className="text-sm font-medium group-hover:text-bubblequest-purple transition-colors">
