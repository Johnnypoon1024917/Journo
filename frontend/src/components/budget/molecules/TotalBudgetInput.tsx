...
export const TotalBudgetInput: React.FC<TotalBudgetInputProps> = ({
  value,
  currency,
  onChange,
  onBlur,
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          } bg-white dark:bg-gray-800 pl-16 pr-4 py-3 text-gray-900 dark:text-white text-lg font-semibold focus:outline-none focus:ring-2 transition-all duration-200`}
          aria-label={label}
          aria-invalid={!!error}
      ...
      {error && (
        <p id="budget-error" className="text-sm text-red-600 dark:text-red-400 flex items-center gap-1">
      ...
      {!error && isValid && (
        <p className="text-xs text-gray-500 dark:text-gray-400">
          Budget set to {currency} {parseFloat(inputValue).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </p>
      )}
    </div>
  );
